...
> 本文档供 AI Agent（如 OpenClaw QClaw）在参与 rust-kinit 项目开发时参考。
...
**当前状态**：功能基本完成，kinit + klist 均可用，已修复上游 bug，Rust 2024 edition 兼容。
...
```
rust-kinit/
├── Cargo.toml                 # Workspace 配置（锁定所有外部依赖版本）
├── README.md                  # 用户文档（用法、构建、示例）
├── AGENTS.md                  # 本文件（开发规范）
├── crates/
│   ├── kinit-kt/             # kinit 命令行工具（bin）
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── main.rs       # CLI 入口，调用 kinit_kt::request_tgt()
│   │       └── lib.rs        # （如有）库接口
│   ├── klist/                # klist 命令行工具（bin）
│   │   ├── Cargo.toml
│   │   └── src/
│   │       └── main.rs       # CLI 入口，读取并格式化 ccache
│   ├── kerbeiros/            # [Vendored] Kerberos AS-REQ/AS-REP 实现
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
...
│   ├── kerberos-asn1/       # [Vendored] Kerberos ASN.1 类型定义
│   ├── kerberos-ccache/     # [Vendored] ccache 文件格式读写
│   │   └── src/
│   │       ├── key_block.rs  # [已修复] KeyBlock::new() etype 硬编码 bug
│   │       └── credential.rs
│   ├── kerberos-constants/  # [Vendored] Kerberos 常量定义（etypes, flags）
│   ├── kerberos-crypto/     # [Vendored] Kerberos 加密算法实现
│   ├── kerberos-keytab/    # [Vendored] keytab 文件格式解析
│   ├── red-asn1/            # [Vendored] ASN.1 DER 编码/解码库
│   └── red-asn1-derive/     # [Vendored] ASN.1 derive 宏（proc-macro）
```
...
### ADR-001: Vendor 所有 himmelblau_* crates

**状态**: 已决定  
**背景**: 上游 `himmelblau_kerbeiros` 存在 bug（etype 使用错误、微软 KDC 不兼容），且无法直接贡献补丁。  
**决策**: 将所有 8 个 `himmelblau_*` crate vendor 到 `crates/` 目录，去掉 `himmelblau_` 前缀，改为 workspace 内 path 依赖。  
**后果**:
...
1. **vendor/ 和 src/ 目录未删除**: 旧目录仍在，可安全删除（需手动确认）
2. **kinit-kt/src/klist.rs 未删除**: 旧文件仍在，已迁移到 `crates/klist/`
3. **错误信息用中文**: 部分错误信息是中文，如需国际化需重构
...
### Q: 为什么 vendor 所有 crates 而不是提交上游 PR？

A: 上游 `himmelblau_kerbeiros` 是 `himmelblau` 项目（Azure AD 集成）的一部分，修复可能影响其他功能。且我们的修复（微软 KDC 兼容）可能不具备通用性。Vendor 后可以自由控制代码质量。

### Q: 为什么不用 `kerberos` crate（crates.io 上的）？

A: `kerberos` crate 不支持 keytab 文件，且 AS-REQ 实现不完整。我们选择 vendor `himmelblau_kerbeiros` 并修复 bug。
